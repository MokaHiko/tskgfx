...
/**/
/* @note Each format in this enum corresponds one-to-one with Vulkan's `VkFormat`, */
...
/**/
/**/
/* @note Each format in this enum corresponds one-to-one with Vulkan's `VkDescriptorType`, */
/* allowing for direct mapping and compatibility when working with Vulkan.*/
...
/* @brief Holds metadata about a texture, including its format, size, and layout properties.*/
/**/
/* This structure encapsulates information required to describe a texture's layout, */
/* memory requirements, and format. It provides essential details for managing textures */
/* within the rendering engine.*/
struct TUSK_API TextureInfo {
	Format format;			//!< Texture format.
	/*uint32_t storage_size;		//!< total amount of bytes required to store texture.*/
...
///
/// This structure contains parameters that define the configuration for the tgfx application.
///
/// @struct AppConfig
...
/// 
/// @var AppConfig::ndt
/// Native display type, a pointer used for platform-specific display information.
/// 
/// @var AppConfig::width
...
///
/// @attention This function releases all resources allocated by the tgfx library.
TUSK_API void shutdown();

static const uint16_t k_invalid_handle = UINT16_MAX;

#define TUSK_HANDLE(name)                                                           \
	struct name { uint16_t idx = tsk::k_invalid_handle; inline operator uint16_t() const { return idx; } }; \
	inline bool is_valid(name handle) { return tsk::k_invalid_handle != handle.idx; }
...
///
/// @note This function will create a program if at least one vsh/fsh path is passed.
...
TUSK_API DescriptorHandle create_descriptor(const char* name, DescriptorType type, uint16_t rh);
...
///
/// @note If data ownership not passed, it must exist for atleast one frame (call to tgfx::frame).
TUSK_API BufferHandle create_uniform_buffer(uint32_t size, void* data);

TUSK_API BufferHandle create_vertex_buffer(VertexLayoutHandle vlh, uint32_t size, void* data);
...
TUSK_API void update(BufferHandle bh, uint32_t offset, uint32_t size, void* data);
...
/*/// @param[in] offset in the buffer to copy the data.*/
/*/// @param[in] data Pointer to the new buffer data.*/
/*TUSK_API void update(BufferHandle bh, uint32_t offset, uint32_t size, void* data, bool manage = false);*/
...
TUSK_API void update(TextureHandle th, uint32_t offset, uint32_t size, void* data);
...
struct DescriptorInfo {
    ...
    char name[256];
    uint16_t resource_handle_index; //!< handle index to the resource this descriptor is bound to.

    inline const bool valid() const {return type != DescriptorType::k_max_enum || resource_handle_index == k_invalid_handle;}
};
...
/* @brief Holds rendering information for a draw call, including transformation matrices */
/* and buffer handles.*/
/**/
/* This structure encapsulates details required to execute a draw call, such as */
/* view and transformation matrices, vertex, index, and instance buffer handles.*/
/* It provides essential data for managing draw operations within the rendering engine.*/
struct RenderDraw {
    RenderDraw() {
	clear();
    }
    ...
    void clear() {
        viewproj_mtx[0]  = 1.0f, viewproj_mtx[1]  = 0.0f, viewproj_mtx[2]  = 0.0f,  viewproj_mtx[3]  = 0.0f;    // 1st column
        viewproj_mtx[4]  = 0.0f, viewproj_mtx[5]  = 1.0f, viewproj_mtx[6]  = 0.0f,  viewproj_mtx[7]  = 0.0f;    // 2nd column
        viewproj_mtx[8]  = 0.0f, viewproj_mtx[9]  = 0.0f, viewproj_mtx[10] = 1.0f,  viewproj_mtx[11] = 0.0f;    // 3rd column
        viewproj_mtx[12] = 0.0f, viewproj_mtx[13] = 0.0f, viewproj_mtx[14] = 0.0f,  viewproj_mtx[15] = 1.0f;    // 4th column

	transform_matrix[0]  = 1.0f, transform_matrix[1]  = 0.0f, transform_matrix[2]  = 0.0f,  transform_matrix[3]  = 0.0f;    // 1st column
        transform_matrix[4]  = 0.0f, transform_matrix[5]  = 1.0f, transform_matrix[6]  = 0.0f,  transform_matrix[7]  = 0.0f;    // 2nd column
        transform_matrix[8]  = 0.0f, transform_matrix[9]  = 0.0f, transform_matrix[10] = 1.0f,  transform_matrix[11] = 0.0f;    // 3rd column
        transform_matrix[12] = 0.0f, transform_matrix[13] = 0.0f, transform_matrix[14] = 0.0f,  transform_matrix[15] = 1.0f;    // 4th column
	
	camera_pos[0] = 0; camera_pos[1] = 0; camera_pos[2] = 0;

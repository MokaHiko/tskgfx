...
struct TextureVk {
    ...
    inline const bool valid() const {return image != VK_NULL_HANDLE && image_view != VK_NULL_HANDLE; }

    void create(VkImageUsageFlags usage, VkExtent3D extent, VkFormat format, VkImageAspectFlags aspect);
...
public:
    ...
    void create(VkBufferUsageFlags usage, VkDeviceSize buffer_size, bool mappable = false);
...
struct ProgramVk {
    ...
    inline const bool valid() const {return pipeline_layout != VK_NULL_HANDLE &&  descriptor_set_layout != VK_NULL_HANDLE; }

    void create(const char* csh_path);

...
inline RenderContextDirtyFlags operator~ (RenderContextDirtyFlags a) { return (RenderContextDirtyFlags)~(int)a; }
inline RenderContextDirtyFlags operator| (RenderContextDirtyFlags a, RenderContextDirtyFlags b) { return (RenderContextDirtyFlags)((int)a | (int)b); }
inline RenderContextDirtyFlags operator& (RenderContextDirtyFlags a, RenderContextDirtyFlags b) { return (RenderContextDirtyFlags)((int)a & (int)b); }
inline RenderContextDirtyFlags operator^ (RenderContextDirtyFlags a, RenderContextDirtyFlags b) { return (RenderContextDirtyFlags)((int)a ^ (int)b); }
inline RenderContextDirtyFlags& operator|= (RenderContextDirtyFlags& a, RenderContextDirtyFlags b) { return (RenderContextDirtyFlags&)((int&)a |= (int)b); }
inline RenderContextDirtyFlags& operator&= (RenderContextDirtyFlags& a, RenderContextDirtyFlags b) { return (RenderContextDirtyFlags&)((int&)a &= (int)b); }
...
public:
    ...
    virtual void update_texture_2d(TextureHandle th, uint32_t offset, uint32_t size, void* data);
    ...
    virtual void create_program(ProgramHandle ph, ShaderHandle vsh, ShaderHandle fsh) = 0;

    virtual void create_descriptor(DescriptorHandle handle, DescriptorType type, uint16_t rh, const char* name) = 0;

    virtual void create_uniform_buffer(BufferHandle bh, uint32_t size, void* data) = 0;
    virtual void create_vertex_buffer(BufferHandle bh, VertexLayoutHandle vlh, uint32_t size, void* data) = 0;
    virtual void create_index_buffer(BufferHandle bh, uint32_t size, void* data) = 0;
    virtual void update_buffer(BufferHandle bh, uint32_t offset, uint32_t size, void* data) = 0;
    virtual void destroy_buffer(BufferHandle bh) = 0;

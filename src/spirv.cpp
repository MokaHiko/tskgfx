...
std::string type_desc_to_string(const SpvReflectTypeDescription& td) {
  ...
  if((td.type_flags & SPV_REFLECT_TYPE_FLAG_VECTOR) == SPV_REFLECT_TYPE_FLAG_VECTOR) {
    out = "vec";
    out += std::to_string(td.traits.numeric.vector.component_count);
    return out;
  }

  if((td.type_flags & SPV_REFLECT_TYPE_FLAG_FLOAT) == SPV_REFLECT_TYPE_FLAG_FLOAT) {
    out += "float";
    out += std::to_string(td.traits.numeric.scalar.width);
    return out;
  }

  if((td.type_flags & SPV_REFLECT_TYPE_FLAG_INT) == SPV_REFLECT_TYPE_FLAG_INT) {
    out += (td.traits.numeric.scalar.signedness > 0) ? "int" : "uint";
    out += std::to_string(td.traits.numeric.scalar.width);
    return out;
  }

  if((td.type_flags & SPV_REFLECT_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER) == SPV_REFLECT_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER) {
...
void parse_type_desc_recursive(const SpvReflectTypeDescription& td, int level = 0) {
  ...
  spdlog::trace("{} {} {} {}", level_tab, td_string, td.struct_member_name ? td.struct_member_name : "", td.type_name ? td.type_name : "");
...
bool parse_spirv(const void* spirv_code, size_t spirv_nbytes, 
                ...
                VkPushConstantRange* pc_ranges, uint32_t* n_pc_ranges) {
...

  {
    ...
    SpvReflectInterfaceVariable** interface_variables = (SpvReflectInterfaceVariable**)malloc(var_count * sizeof(SpvReflectInterfaceVariable*));
    result = spvReflectEnumerateInterfaceVariables(&module, &var_count, interface_variables);
    assert(result == SPV_REFLECT_RESULT_SUCCESS);
...

  {
    ...
    if(bindings != nullptr) {
      SpvReflectDescriptorBinding** ds_bindings = (SpvReflectDescriptorBinding**)malloc(*n_bindings * sizeof(SpvReflectDescriptorBinding*));
      result = spvReflectEnumerateDescriptorBindings(&module, n_bindings, ds_bindings);
      assert(result == SPV_REFLECT_RESULT_SUCCESS);

      for(uint32_t i = 0; i < *n_bindings; i++) {
        const SpvReflectDescriptorBinding& ds_binding = *ds_bindings[i];
        VkDescriptorType ds_type = static_cast<VkDescriptorType>(ds_binding.descriptor_type);

        bindings[i].binding = ds_binding.binding;
        bindings[i].descriptorCount = ds_binding.count;
        bindings[i].descriptorType = ds_type;

        spdlog::trace("ds({}) binding({}) type({})", ds_binding.set, ds_binding.binding, k_ds_type_to_string[ds_type]);
        const SpvReflectTypeDescription& ds_type_desc = *ds_binding.type_description;
        ...
        switch(ds_type) {
          ...
          case(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER): {
            for(uint32_t member_count = 0; member_count < ds_type_desc.member_count; member_count++) {
              const SpvReflectTypeDescription& member = ds_type_desc.members[member_count];
...

  {
    ...
    if(pc_ranges != nullptr) {
      SpvReflectBlockVariable** pc_blocks = (SpvReflectBlockVariable**)malloc(*n_pc_ranges * sizeof(SpvReflectBlockVariable*));
      result = spvReflectEnumeratePushConstantBlocks(&module, n_pc_ranges, pc_blocks);
      assert(result == SPV_REFLECT_RESULT_SUCCESS);
